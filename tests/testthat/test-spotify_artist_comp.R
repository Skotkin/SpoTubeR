...
testthat::test_that("URL spotify specific works", {
  ...
  testthat::expect_error(spotify_artist_comp('https://www.youtube.com/watch?v=nUsrYVxrDwI&list=RDnUsrYVxrDwI'))
  })

testthat::test_that("URL for artist works", {
  # User inputs a URL to a song instead of an artist
  testthat::expect_error(spotify_artist_comp('https://open.spotify.com/track/65DbTqJKhbwqYbZ1Okr0rc?si=4e9a52796fff470f'))
  })
...
testthat::test_that("less common artists works", {
  # checking that the function still works for artists with less than 10 top songs
  testthat::expect_no_error(spotify_artist_comp('https://open.spotify.com/artist/7zPS0577bqv2gQDSDa34Gf?si=844m5H6FRYSatUwqbnzSaA'))
})
